...
type FilterState = 'all' | 'Open' | 'Locked' | 'Drawing' | 'AwaitingRandomness' | 'Completed' | 'Expired' | 'Cancelled'

export function AdminLotteryTable({ lotteries, isLoading }: AdminLotteryTableProps) {
  const [searchTerm, setSearchTerm] = useState('')
  const [statusFilter, setStatusFilter] = useState<FilterState>('all')
  const [sortBy, setSortBy] = useState<'created' | 'drawTime' | 'tickets' | 'prizePool'>('created')
  ...
  const filteredAndSortedLotteries = useMemo(() => {
    ...
    filtered.sort((a, b) => {
      switch (sortBy) {
        case 'created':
          return new Date(b.drawTime * 1000).getTime() - new Date(a.drawTime * 1000).getTime()
        case 'drawTime':
          return new Date(a.drawTime * 1000).getTime() - new Date(b.drawTime * 1000).getTime()
        case 'tickets':
    ...
    return (
      <div className="flex items-center gap-2">
        ...
        <Button
          variant="ghost"
          size="sm"
          onClick={() => window.open(`https://explorer.solana.com/address/${lottery.winningNumbers}?cluster=devnet`, '_blank')}
        >
        ...
        <div className="flex flex-col md:flex-row gap-4 mt-4">
          ...
          <Select value={statusFilter} onValueChange={(value) => setStatusFilter(value as FilterState)}>
            ...
            <SelectContent>
              ...
              <SelectItem value="AwaitingRandomness">Awaiting Randomness</SelectItem>
            ...
            <Table>
              ...
              <TableBody>
                {filteredAndSortedLotteries.map((lottery) => {
                  const commission = lottery.totalTickets * lottery.ticketPrice * 0.02
                  return (
                    <TableRow key={lottery.address} className="hover:bg-gray-50">
