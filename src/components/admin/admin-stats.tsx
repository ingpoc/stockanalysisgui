...
export function AdminStats({ lotteries, isLoading }: AdminStatsProps) {
  const stats = useMemo(() => {
    ...
    const totalTicketsSold = lotteries.reduce(
      (sum, lottery) => sum + lottery.totalTickets,
      0
    );
    const totalPrizePool = lotteries.reduce(
      (sum, lottery) => sum + lottery.prizePool,
      0
    );
    const activeLotteries = lotteries.filter(l => {
      const stateKey =
        typeof l.state === 'object' && l.state
    ...
    const totalRevenue = lotteries.reduce(
      (sum, lottery) => sum + lottery.totalTickets * lottery.ticketPrice,
      0
    );
    ...
    return {
      ...
      activeLotteries,
      completedLotteries,
      avgTicketsPerLottery:
        lotteries.length > 0 ? totalTicketsSold / lotteries.length : 0,
      totalRevenue,
    };
  }, [lotteries]);
