...
export function TreasuryDashboard({ lotteries, isLoading }: TreasuryDashboardProps) {
  const treasuryData = useMemo(() => {
    ...
    const monthlyBreakdown = commissionsByLottery.reduce((acc, item) => {
    ...
    return {
      ...
      claimedCommissions,
      pendingCommissions,
      commissionsByLottery: commissionsByLottery.sort((a, b) => 
        new Date(b.createdAt * 1000).getTime() - new Date(a.createdAt * 1000).getTime()
      ),
  ...
  const commissionPercentage = treasuryData.totalCommissions > 0 
  ...
  return (
    <div className="space-y-12">
      ...
      <div className="grid grid-cols-3 gap-16">
        <div>
          <div className="text-xs text-gray-400 uppercase tracking-wider mb-2">TOTAL COMMISSIONS</div>
        ...
        <div>
          <div className="text-xs text-gray-400 uppercase tracking-wider mb-2">AVAILABLE TO CLAIM</div>
          <div className="text-3xl font-light text-gray-900 font-mono">
            {formatUSDC(treasuryData.claimableAmount)}
          </div>
          <div className="text-xs text-gray-400 mt-1">From completed lotteries</div>
        </div>
        <div>
          <div className="text-xs text-gray-400 uppercase tracking-wider mb-2">PENDING COMMISSIONS</div>
          <div className="text-3xl font-light text-gray-900 font-mono">
            {formatUSDC(treasuryData.pendingCommissions)}
          </div>
          <div className="text-xs text-gray-400 mt-1">From active lotteries</div>
        </div>
      </div>
      ...
      {treasuryData.monthlyBreakdown.length > 0 && (
        <div>
          <div className="text-xs text-gray-400 uppercase tracking-wider mb-8">MONTHLY BREAKDOWN</div>
          <div className="space-y-4">
            {treasuryData.monthlyBreakdown.slice(0, 6).map((month: any) => (
              <div key={month.month} className="flex items-center justify-between py-4 border-b border-gray-100 last:border-b-0">
      ...
      <div>
        <div className="text-xs text-gray-400 uppercase tracking-wider mb-8">COMMISSION DETAILS BY LOTTERY</div>
