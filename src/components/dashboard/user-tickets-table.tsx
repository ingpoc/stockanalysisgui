...
export function UserTicketsTable({
  ...
  const [sortBy, setSortBy] = useState<
    'purchased' | 'drawTime' | 'prize' | 'state'
  >('purchased');
  ...
  const filteredAndSortedTickets = useMemo(() => {
    ...
    if (statusFilter !== 'all') {
      switch (statusFilter) {
        ...
        case 'lost':
          filtered = filtered.filter(
            ticket => ticket.lotteryState === 'Completed' && !ticket.isWinner
          );
          break;
      }
    }
  ...
  const getStatusBadge = (ticket: UserTicket) => {
    ...
    switch (ticket.lotteryState) {
      ...
      default:
        return <Badge variant='outline'>{ticket.lotteryState}</Badge>;
    }
  };
                  ...
                  >
                    ...
                    <TableCell>
                      <Button
                        ...
                        onClick={() =>
                          window.open(
                            `https://explorer.solana.com/address/${ticket.lotteryId}?cluster=devnet`,
                            '_blank'
                          )
                        }
                      >
