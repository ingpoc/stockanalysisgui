...
export function WinningsSummary({ userTickets, userStats, userBalance, isLoading }: WinningsSummaryProps) {
  if (isLoading) {
    return (
  ...
  const claimableWinnings = winningTickets.filter(ticket => 
    ticket.lotteryState === 'Completed' && ticket.prizeAmount
  )
  ...
  return (
    <div className="space-y-12">
      ...
      <div>
        <div className="text-xs text-gray-400 uppercase tracking-wider mb-8">WINNINGS OVERVIEW</div>
        <div className="grid grid-cols-3 gap-16">
          <div>
            <div className="text-xs text-gray-400 uppercase tracking-wider mb-2">TOTAL WINNINGS</div>
          ...
          <div>
            ...
            <div className={`text-3xl font-light font-mono ${
              ...
              {userBalance.netPosition >= 0 ? '+' : ''}{formatUSDC(Math.abs(userBalance.netPosition))}
            </div>
      ...
      {winningTickets.length > 0 && (
        <div>
          <div className="text-xs text-gray-400 uppercase tracking-wider mb-8">RECENT WINNINGS</div>
            ...
            {winningTickets.slice(0, 5).map((ticket) => (
              ...
              >
                ...
                <div className="text-right">
                  ...
                  <div className="text-xs text-gray-400 mt-1">
                    {ticket.lotteryState === 'Completed' ? 'Claimable' : 'Pending'}
                  </div>
                </div>
              </div>
            ))}
      ...
      {winningTickets.length === 0 && userTickets.length > 0 && (
        <div className="text-center py-12 border border-dashed border-gray-200">
          ...
          <p className="text-xs text-gray-400">
            You have {userTickets.length} tickets purchased. Good luck with upcoming draws!
          </p>
        </div>
      )}
    </div>
