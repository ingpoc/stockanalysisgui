...
export function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const currentTitle =
    PAGE_TITLES[pathname] || 'DECENTRALIZED LOTTERY PLATFORM';
  ...
  return (
    <div className='flex h-screen overflow-hidden bg-white'>
      ...
      >
        <div
          className={`border-b border-gray-200 px-6 py-8 ${isCollapsed ? 'px-4' : ''}`}
        >
          {isCollapsed ? (
        ...
        <div className='flex flex-col flex-1 overflow-y-auto duration-300 ease-linear'>
          ...
          <div
            className={`mt-auto py-8 space-y-2 border-t border-gray-200 ${isCollapsed ? 'px-4' : 'px-6'}`}
          >
            {isAdmin && (
              <SidebarItem
                icon={Shield}
