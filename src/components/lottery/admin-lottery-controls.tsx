...
export function AdminLotteryControls({
  lottery,
  onStateChange,
}: AdminLotteryControlsProps) {
  const [selectedState, setSelectedState] = useState<string>('');
  const { connected } = useWallet();
  const { transitionState, isTransitioning } = useLottery();
    ...
    try {
      if (selectedState === 'Drawing') {
        toast.info('Transitioning to Drawing state', {
          description:
            'This will use the oracle account to generate random numbers for the lottery.',
        });
      } else if (selectedState === 'Cancelled') {
        toast.info('Transitioning to Cancelled state', {
          description:
            'This will use the oracle account to cancel the lottery.',
        });
      }
