...
export function EnhancedLotteryCard({
  lottery,
  onParticipate,
}: LotteryCardProps) {
  const { publicKey } = useWallet();

  const { buyTicket, isBuying } = useLottery();

  const isActive =
    typeof lottery.state === 'object' &&
    lottery.state &&
    'open' in lottery.state;
  ...
  const animateNumberRoll = useCallback(
    (
      element: HTMLElement,
      newValue: number,
  ...
  useEffect(() => {
    if (ticketsRef.current && lottery.totalTickets > 0) {
      animateNumberRoll(ticketsRef.current, lottery.totalTickets, val =>
  ...
  useEffect(() => {
    if (
      progressRef.current &&
      lottery.targetPrizePool &&
      lottery.targetPrizePool > 0
    ) {
    ...
    >
      ...
      <div className='p-6 pb-0'>
        <div className='flex items-baseline justify-between'>
          ...
          <span
            className={`text-xs tracking-wider ${isActive ? 'text-gray-900' : 'text-gray-400'}`}
          >
        ...
        <div className='space-y-4'>
          ...
          <div className='grid grid-cols-2 gap-8'>
            ...
            <div>
              ...
              <div
                ref={prizeRef}
                className='text-xl font-light text-gray-900 font-mono'
              >
      ...
      <div className='px-6 pb-6'>
        {isActive && !isEnded && publicKey ? (
          ...
          >
            {isBuying
              ? 'PROCESSING...'
              : `BUY • ${formatUSDCValue(lottery.ticketPrice)}`}
          </button>
        ) : (
