...
export function useRouletteAudio(): RouletteAudioHook {
  ...
  const playSpinSound = useCallback(() => {
    try {
      ...
      filter.frequency.exponentialRampToValueAtTime(300, audioContext.currentTime + 3)
    ...
    try {
      ...
      for (let i = 0; i < 5; i++) {
        setTimeout(() => {
          ...
          oscillator.frequency.setValueAtTime(1500 - i * 100, audioContext.currentTime)
          oscillator.frequency.exponentialRampToValueAtTime(800 - i * 50, audioContext.currentTime + 0.05)
          
          gainNode.gain.setValueAtTime(masterVolume * (0.3 - i * 0.05), audioContext.currentTime)
  ...
  const playDealerVoice = useCallback((announcement: string) => {
