...
export function useRouletteStats() {
  ...
  const [numberFrequency, setNumberFrequency] = useState<Map<number, number>>(new Map())
  const [currentStreak, setCurrentStreak] = useState({ type: 'none', count: 0 })
  ...
  useEffect(() => {
    localStorage.setItem('roulette-frequency', JSON.stringify(Array.from(numberFrequency.entries())))
  }, [numberFrequency])
  ...
  const recordGameSession = useCallback((
    ...
    setStats(prev => {
      ...
      newStats.winRate = newStats.totalGames > 0 
        ? Math.round((newStats.totalWinnings / (newStats.totalWinnings + newStats.totalLosses)) * 100)
        : 0

      // Update streaks
      if (won && currentStreak.type === 'win') {
        newStats.longestWinStreak = Math.max(prev.longestWinStreak, currentStreak.count)
      } else if (!won && currentStreak.type === 'lose') {
        newStats.longestLoseStreak = Math.max(prev.longestLoseStreak, currentStreak.count)
      }
  ...
  const getSessionsByDateRange = useCallback((days: number): GameSession[] => {
    const cutoffTime = Date.now() - (days * 24 * 60 * 60 * 1000)
    return stats.recentSessions.filter(session => session.timestamp >= cutoffTime)
  ...
  const getAverageWin = useCallback((): number => {
    ...
    return winSessions.reduce((sum, s) => sum + s.payout, 0) / winSessions.length
  }, [stats.recentSessions])
  ...
  const getAverageLoss = useCallback((): number => {
    ...
    return loseSessions.reduce((sum, s) => sum + s.betAmount, 0) / loseSessions.length
  }, [stats.recentSessions])
